...
export class OpenTimestampsSink implements AnchorSink {
  constructor(
    ...
    private readonly stamp: (digest: Buffer) => Promise<Buffer> = (d) => stampHash(d, { timeoutMs: 15_000 }),
  ) {}
...
/** Verify an OTS proof against a chain-head hash. */
export async function verifyOtsAgainstHash(
  otsBase64: string,
  hash: string,
): Promise<{ ok: boolean; pending: boolean; unknown?: boolean; detail?: string }> {
  return verifyProof(Buffer.from(otsBase64, 'base64'), hashPrefixToBuffer(hash))
}
