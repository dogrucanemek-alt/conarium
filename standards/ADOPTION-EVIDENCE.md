...
| Henri Sirkkavaara (Vaara) | The Vaara conformance page carries row #3, Emek Can Doğru / VERAX TEKNOLOJİ LİMİTED ŞİRKETİ, at commit `a209864e`: 45 passed, 0 failed, 1 skipped, 75 cases — the page's own figures, which are counts of different things and not a ratio. Vaara scopes the row itself as a reproduction of the author-supplied checkers and vectors, not an independent implementation of `draft-sirkkavaara-vaara-receipt` and not a validation of its text. That is this project running Vaara's corpus, not Vaara implementing this draft. | https://vaara.io/conformance.html | citation |
