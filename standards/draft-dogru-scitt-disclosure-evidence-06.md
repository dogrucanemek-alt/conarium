...
docname: draft-dogru-scitt-disclosure-evidence-06
submissiontype: IETF
number:
date:
consensus: true
...
A record can be intact and silent about what is missing from it. A report can
be complete and silent about what it examined. These are two failures rather
than one, and they are closed in different places.

This document is about the first. It states what a mediator recorded, what its
...
in its own terms rather than by omission.

The second belongs to coverage attestation, where an examination declares the
...
is under way on this list.

The two compose in one direction and substitute in neither. A coverage
...
Neither layer rescues the other. A reader given one has half an answer to a
question they were not told had two halves.
...
: A result carries `matched` as a count while carrying every other outcome as
  ...
  size. The count is a convenience over material the reader already has.
: That property fails in one identified case, and it is the case this document
  ...
  ({{sec-completeness}}), a reader recomputing from material the Issuer chose
  recomputes the Issuer's answer. The digest is then a check on transcription
  and not on completeness, and the `matched` count inherits that standing.
...
The state below was measured against the published 0.2.38 package rather than
...
One gap remains. The tool's exit codes predate this vocabulary and are still
...
The gap that produced this section's own history is closed. The -04 revision
...
A check now runs this section instead of reading it. Every behavioural statement
...
The first thing it caught was the sentence in the paragraph above. From the
...
at least demonstrated the failure mode it was written for.

Two limits, stated rather than left to be found. The check pins the behaviour
...
A second check covers the conformance class rather than this document. Every
outcome the result statement can carry has a case that produces it, and each
independent ground for `indeterminate` has a case of its own. The list of
...
from it — and a class that has lost coverage still passes.
