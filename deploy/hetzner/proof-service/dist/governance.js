...
function match(pattern, value) {
    ...
    if (p === '*' || p === '*.*')
        return true;
    if (p.endsWith('.*'))
        return v.startsWith(p.slice(0, -1)); // "billing.*" -> "billing."
    if (p.startsWith('*.'))
        return v.endsWith(p.slice(1)); // "*.email" -> ".email"
    if (p.endsWith('*'))
...
export class Governance {
    ...
    allowsTable(qualified) {
        const { allowTables, denyTables } = this.policy;
        if (!this.isSchemaQualifiedTable(qualified))
            return false;
        if (denyTables?.some(p => match(p, qualified)))
            return false;
    ...
    allowsConnector(connectorName) {
        const { allowConnectors, denyConnectors } = this.policy;
        if (denyConnectors?.some(p => match(p, connectorName)))
            return false;
    ...
    guardQuery(sql) {
        ...
        for (const tok of WRITE_TOKENS) {
            ...
            if (regex.test(norm))
                this.deny(emptyState, `Blocked write operation: ${tok.trim()}`);
        }
        ...
        try {
            ...
            this.deny(emptyState, `Failed to parse SQL: ${err.message}`);
        }
        if (ast.length > 1) {
    ...
    redact(result, aliases = {}, metadata) {
        ...
        const rows = result.rows.map(row => {
            ...
            for (const key of Object.keys(out)) {
                ...
                const maskColMatch = masks.some(m => match(m, qualifiedSource) || match(m, sourceKey) || match(m, key) ||
    ...
    metadataFrom(state, opts = {}) {
        return {
            ...
            maskedFields: [...(opts.maskedFields ?? new Set())].sort(),
    ...
    metadataFromMetadata(metadata, opts) {
        return {
    ...
    analyzeRead(statement, state, cteNames, cteOutputs = new Map()) {
    ...
    analyzeSelect(statement, state, cteNames, cteOutputs) {
        const scope = { sources: new Map() };
        const maskedOutputs = new Set();
        const aliases = {};
        ...
        for (const expr of statement.groupBy ?? [])
            this.collectExpr(expr, state, scope, cteNames, cteOutputs);
        for (const order of statement.orderBy ?? [])
            this.collectOrderBy(order, state, scope, cteNames, cteOutputs);
        ...
        columns.forEach((column, index) => {
            ...
            if (!this.expressionReferencesMasked(column.expr, scope, cteNames, cteOutputs))
                return;
            ...
            for (const name of output.names)
                maskedOutputs.add(name);
        });
        return { maskedOutputs, aliases };
    }
    collectFrom(from, state, scope, cteNames, cteOutputs) {
        if (from.type === 'table') {
            ...
            if (!from.name.schema && cteNames.has(tableName)) {
                ...
                if (from.join?.on)
                    this.collectExpr(from.join.on, state, scope, cteNames, cteOutputs);
                return;
            }
            if (!from.name.schema) {
        ...
        if (from.type === 'call') {
            ...
            for (const arg of from.args)
                this.collectExpr(arg, state, scope, cteNames, cteOutputs);
            for (const order of from.orderBy ?? [])
                this.collectOrderBy(order, state, scope, cteNames, cteOutputs);
            if (from.filter)
                this.collectExpr(from.filter, state, scope, cteNames, cteOutputs);
            if (from.withinGroup)
                this.collectOrderBy(from.withinGroup, state, scope, cteNames, cteOutputs);
            if (from.over?.orderBy)
                for (const order of from.over.orderBy)
                    this.collectOrderBy(order, state, scope, cteNames, cteOutputs);
            if (from.over?.partitionBy)
                for (const expr of from.over.partitionBy)
                    this.collectExpr(expr, state, scope, cteNames, cteOutputs);
            if (from.join?.on)
                this.collectExpr(from.join.on, state, scope, cteNames, cteOutputs);
        }
    }
    addSource(scope, alias, source) {
        scope.sources.set(alias, source);
    }
    enforceTableAccess(qualified, state) {
    ...
    collectOrderBy(order, state, scope, cteNames, cteOutputs) {
        this.collectExpr(order.by, state, scope, cteNames, cteOutputs);
    }
    collectExpr(expr, state, scope, cteNames, cteOutputs) {
        switch (expr.type) {
            case 'call':
                ...
                for (const order of expr.orderBy ?? [])
                    this.collectOrderBy(order, state, scope, cteNames, cteOutputs);
                if (expr.filter)
                    this.collectExpr(expr.filter, state, scope, cteNames, cteOutputs);
                if (expr.withinGroup)
                    this.collectOrderBy(expr.withinGroup, state, scope, cteNames, cteOutputs);
                if (expr.over?.orderBy)
                    for (const order of expr.over.orderBy)
                        this.collectOrderBy(order, state, scope, cteNames, cteOutputs);
                if (expr.over?.partitionBy)
                    for (const item of expr.over.partitionBy)
                        this.collectExpr(item, state, scope, cteNames, cteOutputs);
                return;
            case 'cast':
            ...
            case 'case':
                ...
                if (expr.else)
                    this.collectExpr(expr.else, state, scope, cteNames, cteOutputs);
                return;
            case 'list':
            case 'array':
            ...
            case 'overlay':
                ...
                this.collectExpr(expr.from, state, scope, cteNames, cteOutputs);
                if (expr.for)
                    this.collectExpr(expr.for, state, scope, cteNames, cteOutputs);
                return;
            case 'substring':
                this.collectExpr(expr.value, state, scope, cteNames, cteOutputs);
                if (expr.from)
    ...
    expressionReferencesMasked(expr, scope, cteNames, cteOutputs) {
        switch (expr.type) {
            case 'ref':
    ...
    refReferencesMasked(ref, scope) {
        ...
        if (ref.table?.name) {
            ...
            if (source?.kind === 'derived')
                return source.maskedColumns?.has(column) ?? false;
        ...
        for (const source of scope.sources.values()) {
            if (source.kind === 'derived' && source.maskedColumns?.has(column))
                return true;
            if (source.kind === 'table' && source.schema && source.table && this.matchesMaskedColumn(source.schema, source.table, column))
                return true;
        }
        return this.matchesMaskedColumn(undefined, undefined, column);
    }
    matchesMaskedColumn(schema, table, column) {
        const masks = this.policy.maskColumns ?? [];
        const candidates = new Set([column]);
        if (table)
            candidates.add(`${table}.${column}`);
        if (schema && table)
            candidates.add(`${schema}.${table}.${column}`);
        if (schema)
            candidates.add(`${schema}.${column}`);
        return masks.some(mask => [...candidates].some(candidate => match(mask, candidate)));
    }
    outputNamesForColumn(column, index, scope) {
        if (column.alias)
            return { names: [column.alias.name], reliable: true };
        const expr = column.expr;
        if (expr.type === 'ref') {
            if (expr.name === '*') {
                const names = [...scope.sources.values()].flatMap(source => [...(source.maskedColumns ?? new Set())]);
    ...
    limitTarget(statement) {
        if (statement.type === 'select')
            return statement;
        if (statement.type === 'with' || statement.type === 'with recursive')
            return this.limitTarget(statement.in);
        return undefined;
    }
}
export class ApiGovernance {
    ...
    allowsTool(toolName) {
        ...
        if (denyTools?.some(p => match(p, toolName)))
            return false;
        if (allowTools && allowTools.length > 0)
            return allowTools.some(p => match(p, toolName));
        return true;
    }
    redactResponse(data) {
        if (!data)
    ...
    maskPii(obj) {
        if (!obj)
            return obj;
        if (typeof obj === 'string')
            return this.applyRegexMasks(obj);
        if (Array.isArray(obj))
