...
assert.equal(loading.empty.textContent, 'Loading…')
...
assert.equal(once.chain.textContent, 'chain intact')
assert.ok(once.tbody.children[0].children.some((c) => c.textContent === 'svc'))

const empty = ui()
assert.equal(panel.render(empty, { items: [], empty: 'no receipts yet' }, make()), 'empty')
assert.equal(empty.empty.hidden, false)
assert.match(empty.empty.textContent, /no receipts yet/)
