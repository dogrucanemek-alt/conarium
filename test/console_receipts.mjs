...
assert.ok(one.json.html.includes('Makbuz rec-aaa'))
assert.ok(one.json.html.includes('Sınırlar'))
...
assert.ok(brokenHtml.body.includes('kırık (satır 2)'))
assert.ok(!brokenHtml.body.includes('zincir sağlam'))
...
assert.match(empty.json.empty, /henüz makbuz yok/)
