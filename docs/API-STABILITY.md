...
| `conarium-verify` | `--pubkey` (repeatable) `--anchor-check` `--anchors` `--expect-seq-from` `--expect-count` `--expect-last-hash` `--json` `--help` | **candidate** (exit codes + flags in RECEIPT-SPEC) |
