...
\begin{itemize}
\item npm package \texttt{@conarium-ai/core}, version \texttt{0.2.46} (declared, from the repository \texttt{package.json} at the commit that carries this preprint).
\item Individual Internet-Draft \texttt{draft-dogru-scitt-disclosure-evidence-06}, title \emph{Transformation Evidence and Coverage Reconciliation for Auditable Data Disclosure} (declared, from the draft front matter)~\cite{draft-dogru-scitt-06}. It defines Transformation Evidence and Coverage Reconciliation as evidence payloads for registration as signed statements on a transparency service as described in RFC~9943.
