...
% Each of these is written once and read twice: on the title page, and in the
% document information dictionary the PDF carries. A copy of this file that has
% travelled away from its record has only the second one to say what it is.
\newcommand{\papertitlefirst}{Two ledgers, one window:}
\newcommand{\papertitlesecond}{integrity of a mediator's record does not establish its completeness}
\newcommand{\paperauthorname}{Emek Can Doğru}
\newcommand{\paperaffiliation}{VERAX TEKNOLOJİ LİMİTED ŞİRKETİ, Turkey}
\newcommand{\papersubject}{A conditional impossibility result and two normative principles for judging a mediator's record against a second book}
\newcommand{\paperkeywords}{audit, completeness, coverage reconciliation, provenance, receipts, transparency, two clocks}

\title{\papertitlefirst\\
\papertitlesecond}
\author{\paperauthorname\\
\paperaffiliation}
% A preprint that carries no date leaves a reader holding a copy with no way to
% tell which draft it is. This is the day these sources were finalised, not the
% day the record is deposited; those are allowed to differ and usually do.
\date{Draft of 24 August 2026}

\hypersetup{
  pdftitle={\papertitlefirst{} \papertitlesecond},
...
% The abstract is not a numbered section. Numbering it makes the introduction
% section 2 and shifts every cross-reference in the paper by one, which is not
% how the field reads a preprint.
\begin{abstract}
...
% A copy of this file in a mailbox or a reference manager has left the record
% page, and its terms, behind. The licence travels with the file.
\begin{center}
\copyright{} 2026 \paperauthorname. Licensed under CC BY 4.0
...
Let $G$ be a mediator producing record $R$ of events it observes, and let $E$ be the multiset of occurrences that actually happened. Under assumptions A1--A7 above, if there exists a path by which an occurrence can happen without $G$ observing it, then \textbf{there exist two worlds $W_0$, $W_1$ differing in whether that occurrence happened, for which $R$ is identical.} Consequently no judgement whose only input is $R$ can distinguish them.
...
\section{Principle 2 --- Provenance Ceiling (2a) and Undeclared Does Not Decide (2b)}

\textbf{Principle 2a (Provenance Ceiling, per bound).}
...
\textbf{Principle 2b (Undeclared Does Not Decide).}
...
The structure is a product order, not a total order. The pair (\textit{measured multiplicity}, \textit{undeclared skew}) is incomparable with (\textit{undeclared multiplicity}, \textit{measured skew}) (declared, illustrative). Collapsing both pairs to a single meet would be a safe over-coarsening; it would also hide which bound is carrying the result.
...
\begin{tabular}{@{}p{0.15\linewidth}p{0.37\linewidth}p{0.34\linewidth}@{}}
\toprule
...
The ceiling applies to world-claims, not to account-claims. Principle 2b forces $\mathit{indeterminate}$ as an outcome class when a required bound is undeclared. It does not force every item into that class. An operator-declared correspondence may still attribute. It may not print that attribution as a measurement.
...
\subsection{\texorpdfstring{Why undecided is mandatory, and why ``clean'' is forbidden}{Why undecided is mandatory, and why “clean” is forbidden}}
